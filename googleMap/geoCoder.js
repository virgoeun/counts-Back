...
router.get("/challenge", (req, res) => {
  ...
  geocoder.geocode(address, function (err, geoRes) {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "An error occurred while geocoding the address." });
    }
    ...
    if (geoRes.length > 0) {
      ...
      performNearbySearch(location, radius, keyword)
        ...
        .catch((error) => {
          ...
          res
            .status(500)
            .json({
              error: "An error occurred while performing the nearby search.",
            });
        });
    } else {
      ...
      res
        .status(404)
        .json({ error: "No geocoding results found for the address." });
    }
  });
});
